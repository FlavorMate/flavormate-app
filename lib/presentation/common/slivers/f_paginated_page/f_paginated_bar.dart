...
class FPaginatedBar<T> extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<FPaginatedBar> createState() => _FPaginatedBarState();
}

class _FPaginatedBarState extends ConsumerState<FPaginatedBar> {
  int _totalPages = 1;
  ...
  List<Widget> _buildPaginationButtons(int currentPage) {
    ...
    return paginationLabels.map((label) {
      final pageIndex = label == FPaginatedBar.ellipsis
          ? -1
          : int.parse(label) - 1;
      return FPageableButton(
        label: label,
        width: FPaginatedBar.buttonWidth,
  ...
  void _handlePageChange(String label, int pageIndex) {
    if (label != FPaginatedBar.ellipsis) {
      setPage(ref, pageIndex);
    }
  }

  List<String> _createPaginationLabels(int currentPage) {
    if (_totalPages <= FPaginatedBar.maxVisiblePages) {
