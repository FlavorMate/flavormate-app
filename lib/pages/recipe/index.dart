...
class _RecipePageState extends ConsumerState<RecipePage> {
  ...
  void initState() {
    super.initState();
    ref.listenManual(pRecipeProvider(widget.recipeId), fireImmediately: true, (
      _,
      next,
    ) {
    ...
    return RScaffold(
      ...
      builder: (_, recipe) => LayoutBuilder(
        builder: (_, constraints) {
          ...
          return RStruct(bringProvider, (context, isBringEnabled) {
            return TResponsive(
  ...
  Future<void> share(BuildContext context) async {
    ...
    SharePlus.instance.share(ShareParams(
      text: '${L10n.of(context).p_recipe_share(recipe.label)} 🍽️\n$url'
    ));
  }
  ...
  Future<void> transfer() async {
    final id = await showDialog<int>(
      context: context,
      builder: (_) => const ChangeOwnerDialog(),
    );
    if (id == null) return;

    final response = await ref.read(pApiProvider).recipesClient.changeOwner(
      widget.recipeId,
      {'owner': id},
    );
    if (response) {
  ...
  Future<void> delete() async {
    final response = await showDialog<bool>(
      context: context,
      builder: (_) =>
          TConfirmDialog(title: L10n.of(context).d_recipe_delete_title),
    );
    if (!response!) return;
    if (await ref
        .read(pApiProvider)
